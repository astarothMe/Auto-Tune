...
public class AutoTuneTransactionCommand implements CommandExecutor {
    ...
    @Override
    public boolean onCommand(@NotNull CommandSender sender, Command command, @NotNull String transactions, @NotNull String[] args) {
    ...
    public void loadTransactionGUI (Player player, Transaction[] transactions, int page, String queried_player) {
        ...
        for (Transaction transaction : transactions){
            ...
            try{
                item_stack = new ItemStack(Objects.requireNonNull(Material.matchMaterial(item_name)), amount);
            }
            ...
            ItemMeta item_meta = Objects.requireNonNull(item_stack).getItemMeta();
            Objects.requireNonNull(item_meta).setDisplayName(ChatColor.GOLD + item_name);
            item_meta.setLore(generateLoreFromTransaction(transaction));
            item_stack.setItemMeta(item_meta);
            GuiItem gItem = new GuiItem (item_stack, event -> event.setCancelled(true));
            pane.addItem(gItem);
        }
        GUI.addPane(pane);
    ...
    public StaticPane createNextPagePane (int page, Player querying_player, String queried_player) {
        ...
        Objects.requireNonNull(item_meta).setDisplayName(ChatColor.DARK_PURPLE + "Next");
		item_meta.setLore(Collections.singletonList(ChatColor.GRAY + "Page " + ChatColor.WHITE + (page + 1)));
		item_stack.setItemMeta(item_meta);
    ...
    public StaticPane createBackPagePane (int page, Player querying_player, String queried_player) {
        ...
        ItemStack item_stack = new ItemStack(Material.ARROW);
        ItemMeta item_meta = item_stack.getItemMeta();
        Objects.requireNonNull(item_meta).setDisplayName(ChatColor.DARK_PURPLE + "Back");
		item_meta.setLore(Collections.singletonList(ChatColor.GRAY + "Page " + ChatColor.WHITE + (page - 1)));
		item_stack.setItemMeta(item_meta);
        GuiItem gItem = new GuiItem(item_stack, event ->{
            event.setCancelled(true);
