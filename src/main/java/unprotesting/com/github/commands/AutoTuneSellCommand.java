...
public class AutoTuneSellCommand implements CommandExecutor {
    ...
    public final Integer menuRows = Config.getMenuRows();

    @Override
    public boolean onCommand(@NotNull CommandSender sender, Command command, @NotNull String sell, @NotNull String[] args) {
        ...
        if (command.getName().equalsIgnoreCase("sell")) {
            ...
            if (args.length == 0){
                if (p.hasPermission("at.sell") || p.isOp()){loadSellGUI(p, sender);}
    ...
    public static void roundAndGiveMoney(Player player, double moneyToGive, Boolean autoSell) {
		...
		if (moneyToGive > 0) {
            ...
            if (autoSell){
                Main.tempdatadata.putIfAbsent(player.getUniqueId().toString(), 0.0);
