...
public final class Main extends JavaPlugin implements Listener {
  ...
  static final File playerdata = new File("plugins/Auto-Tune/", "playerdata.yml");
  ...
  public static final ConcurrentMap<UUID, ConcurrentHashMap<String, Integer>> maxBuyMap = new ConcurrentHashMap<>();
  public static final ConcurrentMap<UUID, ConcurrentHashMap<String, Integer>> maxSellMap = new ConcurrentHashMap<>();
  ...
  public static ConcurrentHashMap<Integer, OutlinePane> pageArray = new ConcurrentHashMap<>();
  ...
  @Getter
  public static ConcurrentHashMap<String, ItemPriceData> itemPrices = new ConcurrentHashMap<>();
  ...
  public void onEnable() {
    ...
    setupDataFiles();
    if (tempdatadata.isEmpty() || tempdatadata.get("SellPriceDifferenceDifference") == null) {
    ...
    setupMaxBuySell();
    Objects.requireNonNull(this.getCommand("at")).setExecutor(new AutoTuneCommand());
    Objects.requireNonNull(this.getCommand("shop")).setExecutor(new AutoTuneGUIShopUserCommand());
    Objects.requireNonNull(this.getCommand("sell")).setExecutor(new AutoTuneSellCommand());
    Objects.requireNonNull(this.getCommand("autosell")).setExecutor(new AutoTuneAutoSellCommand());
    Objects.requireNonNull(this.getCommand("loan")).setExecutor(new AutoTuneLoanCommand());
    Objects.requireNonNull(this.getCommand("atconfig")).setExecutor(new AutoTuneAutoTuneConfigCommand());
    Objects.requireNonNull(this.getCommand("gdp")).setExecutor(new AutoTuneGDPCommand());
    Objects.requireNonNull(this.getCommand("buy")).setExecutor(new AutoTuneBuyCommand());
    Objects.requireNonNull(this.getCommand("transactions")).setExecutor(new AutoTuneTransactionCommand());
    basicVolatilityAlgorithim = Config.getBasicVolatilityAlgorithim();
    priceModel = Config.getPricingModel();
    scheduler = getServer().getScheduler();
    if (Config.isAutoSellEnabled()){
    scheduler.scheduleSyncRepeatingTask(getINSTANCE(), new AutoSellEventHandler(), Config.getAutoSellUpdatePeriod() * 5L,
    ...
    scheduler.scheduleAsyncRepeatingTask(getINSTANCE(), new LoanEventHandler(), 10,
            Config.getInterestRateUpdateRate());
    ...
    scheduler.scheduleAsyncRepeatingTask(getINSTANCE(), new EnchantmentPriceHandler(), 900L *Config.getTimePeriod(), (Config.getTimePeriod()*2400));
  ...
  public static String[] convert(Set<String> setOfString) {
    return setOfString.toArray(String[]::new);
  }
  ...
  public static ConcurrentHashMap<String, Integer> loadMaxStrings(ConcurrentMap<String, ConcurrentHashMap<Integer, Double[]>> mainMap){
    ConcurrentHashMap<String, Integer> maxMap = new ConcurrentHashMap<>();
  ...
  public void SellDifrunnable() {
    new BukkitRunnable() {
      ...
      public void run() {
        ...
        Main.debugLog("Updates: " + updates);
        Main.debugLog("Variation: " + variation);
        Main.debugLog("Changed sell-price-difference by " + updateVariation + " to "
            + Config.getSellPriceDifference());
  ...
  public void loadSections(){
    int sectionAmount = Objects.requireNonNull(getShopConfig().getConfigurationSection("sections")).getKeys(false).size();
    sectionedItems = new Section[sectionAmount];
    int i = 0;
    for (String section : Objects.requireNonNull(getShopConfig().getConfigurationSection("sections")).getKeys(false)){
  ...
  public static void setupDataFiles() {
    ...
    tempdatadata.putIfAbsent("GDP", 0.0);
    topSellers = new ArrayList<>();
    topBuyers = new ArrayList<>();
  }
  ...
  @Deprecated
  public boolean onCommand(@NotNull CommandSender sender, Command testcmd, @NotNull String trade, @NotNull String[] help) {
  ...
  public void loadShopData() {
    ...
    if (!testset.isEmpty()) {
      Main.getINSTANCE();
      for (String key : Objects.requireNonNull(Main.getShopConfig().getConfigurationSection("shops")).getKeys(false)) {
        ...
        if (!map.containsKey(key)) {
          ...
          double temp_a = Objects.requireNonNull(config).getDouble("price");
          Double[] tempDArray = {
            temp_a,
            0.0,
            0.0
          ...
          Double[] > tempMap3 = new ConcurrentHashMap<>();
          tempMap3.put(0, tempDArray);
          map.put(key, tempMap3);
        }
        i++;
      }
    }
  ...
  public void loadShopsFile() {
    ...
    if (testset.isEmpty()) {
      ...
      int i = 0;
      Main.getINSTANCE();
      for (String key : Objects.requireNonNull(getShopConfig().getConfigurationSection("shops")).getKeys(false)) {
        ConfigurationSection config = Objects.requireNonNull(getShopConfig().getConfigurationSection("shops")).getConfigurationSection(key);
        if (config == null) {
        ...
        ConcurrentHashMap < Integer,
        Double[] > start = (new ConcurrentHashMap<>());
        start.put(0, x);
        map.put(key, start);
        debugLog("Loaded shop: " + key + " at price: " + temp_a);
        i++;
      }
      log("Default shops loaded from shop file");

    }
    if (!testset.isEmpty() && getMainConfig().getBoolean("debug-enabled")) {
      int b = testset.size();
      debugLog(b + " Items Loaded: " + testset.toString());
    }
  }
